...
git pasw
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
...
## Add the mean Air T synchrony to the *synchrony_spp data*
```{r}
...
synchrony_spp_mmglm=synchrony_spp %>% 
  select(Correlation, sYNGEO_ID1, sYNGEO_ID2, dist_ntw, mean_pair_BC, Tmean_synchrony, basin_ID, sitepairs1, sitepairs2, Species, env.dist, RaupCrick, Pod.repl, Pod.rich, FunPod.rich, FunPod.repl) %>% 
  filter(complete.cases(.))
```
...
mmglm1=
lmerMultiMember::lmer(Correlation~ scale(sqrt(dist_ntw)) + scale(Tmean_synchrony)+ scale(mean_pair_BC)+scale(env.dist) +
...
```{r}
(AIC(mmglm_null)- AIC(mmglm5b))*log2(exp(1))

anova(mmglm_null, mmglm5b)
```
...
```{r}
 mmglm5_repl=
lmerMultiMember::lmer(Correlation~ sqrt.dist_ntw * Pod.repl +
              (1 | basin_ID) + (1|Species) + (1 | Pairs),
            memberships = list(basin_ID = Wa, Pairs = Wx), REML = T, data=synchrony_spp_mmglm)
...
```{r}
 mmglm5_rich=
lmerMultiMember::lmer(Correlation~ sqrt.dist_ntw * Pod.rich +
              (1 | basin_ID) + (1|Species) + (1 | Pairs),
            memberships = list(basin_ID = Wa, Pairs = Wx), REML = T, data=synchrony_spp_mmglm)
...
# using the *Replacement component has more support for modelling synchrony*
```{r}
AIC(mmglm5_repl, mmglm5_rich)

AIC(mmglm5b_repl, mmglm5b_rich)
...
r2_nakagawa(mmglm5b_repl)
r2_nakagawa(mmglm5b_rich)
...
synchrony_spp %>% 
  ...
  ggplot()+aes(Pod.repl , Correlation)+geom_smooth(method='lm')+xlab('Podani_beta')+ylab('Population synchrony')+
  geom_smooth(data=synchrony_spp, aes(Pod.rich, Correlation), method = 'lm', col='darkred')+xlab('Beta-diversity')+
    annotate('text', x=0.75, y=0.14, label='Richness diff.', col='darkred')+
    annotate('text', x=0.75, y=0.12, label='Replacement', col='blue')+
...
null.inter_BC_mmglm.df %>% 
  ...
  annotate('text', x=19.5, y =0.250, label = 'Observed - low beta-diversity', size=4.5)+ylab('Synchrony-decay intercept')+
...
null.inter_BC_mmglm.df %>% 
  ...
  xlab(NULL)+ylab(NULL)+theme(axis.text.y = element_text(size=10), axis.text.x = element_blank())+
  ylim(c(0.13, 0.25))+
  annotate('text', x=13, y = 0.175, label ='Null distribution', size=5)+
  annotate('text', x=19.6, y =0.14, label = 'Observed - high beta-diversity', size=4.5)+ylab(NULL)+
  theme(axis.title.y = element_text(size=15))
```
